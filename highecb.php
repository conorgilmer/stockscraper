...
<div id="container"></div>
  <script>
    $.getJSON('https://www.quandl.com/api/v3/datasets/ECB/<?php echo $qcode;?>.json?start_date=<?php echo $begin?>&end_date=<?php echo $today;?>&order=asc', function(json) {
        var hiJson = json.dataset.data.map(function(d) {
          return [new Date(d[0]).getTime(), d[1]]
        });

        // Create the chart
        $('#container').highcharts('Currency', {
          rangeSelector: {
            selected: 1
          },
          title: {
            text: '<?php echo $qcode;?> Currency'
          },
          series: [{
            name: '<?php echo $qcode;?>',
            data: hiJson,
